...
public class MyTeamsFragment extends BaseFragment implements MyTeamsView, MatchInfoView {
    ...
    String seriesId = "", matchId = "", visitorteamId = "", localteamId = "", contestId = "", joiningAmount = "", chip = "", userInviteCode = "", teamName = "", teamId = "", cashBonusContribution = "", SeriesGUID = "";
    ...
    private BroadcastReceiver updates_receiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent i) {
            try {
                if (i.getAction().equals(MyTeamsFragment.class.getSimpleName())) {
                    if (i.hasExtra("KEY")) {
    ...
    private OnItemClickListener.OnItemClickCallback onItemClickCallback = new OnItemClickListener.OnItemClickCallback() {
        ...
        public void onItemClicked(View view, final int position) {
            ctvCreateContests.setBackgroundResource(R.drawable.button_solid_bg_theme_active);
            ctvCreateContests.setTextColor(getResources().getColor(R.color.white));
    ...
    private List<MyTeamOutput.DataBean.RecordsBean> mResponse;
    private String join;
    ...
    public void createContests(View view) {
        JoinContestActivityStart(mContext, matchId, adapter.getSelectTeamId(), contestId, joiningAmount, chip, userInviteCode, matchTeamVS);
    }
    ...
    public void JoinContestActivityStart(Context context, String matchId, String teamId, String contestId, String joiningAmount, String chip, String userInviteCode, String matchTeamVS) {
    ...
    public void init() {
        ...
        if (getArguments() != null) {
            ...
            //teamsVS.setText(getArguments().getString("teamsVSStr"));
        }
        LocalBroadcastManager.getInstance(mContext).registerReceiver(updates_receiver, new IntentFilter(MyTeamsFragment.class.getSimpleName()));
        ...
        ctvCreateContests.setBackgroundResource(R.drawable.button_solid_bg_theme_inactive);
        ctvCreateContests.setTextColor(getResources().getColor(R.color.white));
        ...
        adapter = new MyTeamsAdapter(TextUtils.isEmpty(contestId)
                ? R.layout.list_item_teams : R.layout.list_item_select_teams, localteamId, getActivity(),
                teamsRecord, onItemClickCallback,
        ...
        if (!TextUtils.isEmpty(contestId) && SWITCH) {
            ...
            ctvSwitchTeam.setBackgroundResource(R.drawable.button_solid_bg_theme_inactive);
    ...
    public void onLoadingSuccess(MyTeamOutput response) {
        if (isLayoutAdded() && mRecyclerView != null && response.getData().getRecords() != null) {
            ...
            ctvSwitchTeam.setBackgroundResource(R.drawable.button_solid_bg_theme_active);
            ...
            Log.i("ctvCreateTeam", "ctvCreateTeam: " + response.getData().getRecords().size());

            callMatchDetail(matchId, statusId);
    ...
    public void onMatchSuccess(MatchDetailOutPut responseLogin) {
        ...
        if (responseLogin.getData() != null) {
            ...
            adapter.localTeamGUID = responseLogin.getData().getTeamNameShortLocal();
    ...
    /*@Override
    public void onShowMatchLoading() {
        if (mProgressDialog == null) mProgressDialog = new ProgressDialog(mContext);
        mProgressDialog.show();
    }

    @Override
    public void onHideMatchLoading() {
        if (mProgressDialog != null) mProgressDialog.dismiss();
    }*/
        ...
        try {
            if (TimeUtils.isThisDateValid(details.getData().getMatchStartDateTime(), "yyyy-MM-dd HH:mm:ss")) {
                ...
                if (TimeUnit.MILLISECONDS.toHours(diff) > Constant.SHOW_TIME_LIMIT_HRS) {
                    customTextViewMatchStatus.setText(TimeUtils.getMatchDateOnly(details.getData().getMatchDate()));
                } else {
                    try {
                        ...
                        countDownTimer = new CountDownTimer(diff, TimeUnit.SECONDS.toMillis(1)) {
                            public void onTick(long millisUntilFinished) {
                                if (customTextViewMatchStatus != null) {
                                    customTextViewMatchStatus.setText(TimeUtils.getRemainsTime(millisUntilFinished));
                                } else {
                                    if (countDownTimer != null) {
                                        countDownTimer.cancel();
                                    }
                            ...
                            public void onFinish() {
                                if (customTextViewMatchStatus != null) {
                                    customTextViewMatchStatus.setText(AppUtils.getStrFromRes(R.string.in_progress));
