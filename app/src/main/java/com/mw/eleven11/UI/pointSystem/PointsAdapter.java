...
public class PointsAdapter extends RecyclerView.Adapter<PointsAdapter.MyViewHolder> {
    ...
    public PointsAdapter(Context mContext, String type) {
    ...
    public void onBindViewHolder(final MyViewHolder holder, final int position) {
        ...
        if (type.equalsIgnoreCase("PointsT10")) {
            ...
            if (responseBeen.get(position).getPointsValue().contains("-")) {
                holder.player_point.setText(responseBeen.get(position).getPointsValue());
                holder.player_point.setTextColor(mContext.getResources().getColor(R.color.red));
            }else {
                holder.player_point.setText("+" + responseBeen.get(position).getPointsValue());
                holder.player_point.setTextColor(mContext.getResources().getColor(R.color.green));
    ...
    public void addAllItem(List<PointsList.PointsData.AllCricketPoints> beanList) {
        ...
        for (int i = 0; i < beanList.size(); i++) {
            if (type.equalsIgnoreCase("PointsTEST")&&beanList.get(i).getPointsTypeDescprition().length()<50) {
                addItem(beanList.get(i));
            }else if(type.equalsIgnoreCase("PointsODI")||type.equalsIgnoreCase("PointsT20")||type.equalsIgnoreCase("PointsT10")
            ||type.equalsIgnoreCase("football")) {
                addItem(beanList.get(i));
            }
        }
    }
