...
public class PointsBowlAdapter extends RecyclerView.Adapter<PointsBowlAdapter.MyViewHolder> {
    ...
    public PointsBowlAdapter(Context mContext, String type) {
    ...
    public void onBindViewHolder(final MyViewHolder holder, final int position) {
        ...
        if (type.equalsIgnoreCase("PointsT10")) {
            ...
            if (responseBeen.get(position).getPointsTEST().equalsIgnoreCase("0.0")||responseBeen.get(position).getPointsTypeDescprition().length()>50){
                holder.itemView.setVisibility(View.GONE);
            }else {
                holder.itemView.setVisibility(View.VISIBLE);
